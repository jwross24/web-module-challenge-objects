...
function createMenuItem(myName, myPrice, myCategory) {
  return { name: myName, price: myPrice, category: myCategory }
}
...
const matchaLatte = createMenuItem('Matcha Latte', 4, 'Drinks');
...
reviews.push({ name: "Selina", rating: 2.5, feedback: "Meh." });
console.log(reviews);
...
function getLastReview(reviews) {
  ...
  return getReviewByIndex(reviews, lastIndex);
}
...
function getReviewByRating(reviews, rating) {
  ...
  for (let i = 0; i < reviews.length; i++) {
    ...
    if (rating <= reviewRating && reviewRating < rating + 1) {
